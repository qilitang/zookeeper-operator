...
import (
	...
	"github.com/qilitang/zookeeper-operator/internal/common/options"
	"github.com/qilitang/zookeeper-operator/internal/common/status"
	"github.com/qilitang/zookeeper-operator/internal/operator"
	utils2 "github.com/qilitang/zookeeper-operator/internal/utils"
...
func (r *ZookeeperClusterReconciler) HandleReplicas(ctx context.Context, log logr.Logger, cluster *qilitangtopv1.ZookeeperCluster) error {
	setList := &v1.StatefulSetList{}
	matchLabels := utils2.CopyMap(operator.NewClusterLabel(cluster))
	...
	shareResource := operator.ClusterSubResources{Cluster: cluster, Client: r.Client, Log: log}
	reference := options.ReferenceToOwner(r.OwnerReference, cluster)
	err := options.SyncShareResources(r.Client, reference, []options.ResourcesCreator{
	...
	for serverIndex := 0; serverIndex < nums; serverIndex++ {
		...
		if errors.IsNotFound(err) {
			err = options.SyncShareResources(r.Client, reference,
			...
			if err := r.pollStatefulSet(ctx, log, cluster, newSet.Name); err != nil {
		...
		updateFlag, err := r.DoSyncSetUpdate(ctx, log, set, cluster)
		if err != nil {
			return err
		}
		if updateFlag {
		...
		if cluster.Spec.Stop {
			//轮流关闭sts
			continue
		}
		if err := r.pollStatefulSet(ctx, log, cluster, set.Name); err != nil {
	...
	// 释放资源
	if err := r.deleteReplicasResources(ctx, log, cluster); err != nil {
		return err
	}
	return nil
}

func (r *ZookeeperClusterReconciler) UpdateCluster(ctx context.Context, l logr.Logger, cluster *qilitangtopv1.ZookeeperCluster) error {
	newCluster := &qilitangtopv1.ZookeeperCluster{}
	...
	zookeeperStatus := operator.ZookeeperClusterResourcesStatus{
		...
		Client:        r.Client,
		Log:           l.WithName("ZookeeperResourcesStatus"),
		Cluster:       newCluster,
	}
...
func (r *ZookeeperClusterReconciler) DoSyncSetUpdate(ctx context.Context, l logr.Logger, set *v1.StatefulSet, cluster *qilitangtopv1.ZookeeperCluster) (updateFlag bool, err error) {
	updateFlag = false
	newSet := set.DeepCopy()
	//replicas := cluster.Spec.Replicas
	checker := &operator.ZookeeperChecker{
		Client: r.Client,
		Log:    l.WithName("ZookeeperChecker"),
	}
...
func (r *ZookeeperClusterReconciler) deleteReplicasResources(ctx context.Context, l logr.Logger, cluster *qilitangtopv1.ZookeeperCluster) error {
	log := l.WithValues("zookeeper cluster", "delete replica")
...
func (r *ZookeeperClusterReconciler) Exec(ctx context.Context, podName string, cluster *qilitangtopv1.ZookeeperCluster, cmd []string) (string, error) {
...
func (r *ZookeeperClusterReconciler) FindLeaderPodName(ctx context.Context, l logr.Logger, cluster *qilitangtopv1.ZookeeperCluster) (string, error) {
	log := l.WithValues("zookeeper cluster ", "find leader pod")
...
func (r *ZookeeperClusterReconciler) reConfigCluster(ctx context.Context, l logr.Logger, cluster *qilitangtopv1.ZookeeperCluster, serverIndex int) error {
	if serverIndex == 0 {
		return nil
	}
	// findLeader pod
	leaderPodName, err := r.FindLeaderPodName(ctx, l, cluster)
	if err != nil {
		return err
	}
	cmd := []string{
...
//func (r *ZookeeperClusterReconciler) checkReplicaRole(ctx context.Context, cluster *qilitangtopv1.ZookeeperCluster, set *v1.StatefulSet) error {
//	cmd := []string{
//		"sh",
//		"-c",
//		"echo stat | nc localhost 2181",
...
//func (r *ZookeeperClusterReconciler) checkAllNodeRole(ctx context.Context, cluster *qilitangtopv1.ZookeeperCluster) error {
...
func (r *ZookeeperClusterReconciler) pollStatefulSet(ctx context.Context, log logr.Logger, cluster *qilitangtopv1.ZookeeperCluster, setName string) error {
	ctx, cancel := context.WithCancel(ctx)
	defer cancel()
	if cluster.Spec.Stop {
		log.V(2).Info("zookeeper cluster is stop")
		return nil
	}
	succeeded := r.waitStsSuccess(ctx, log, cluster, setName)
	if !succeeded {
		return fmt.Errorf("sts is not success")
	}
	log.Info("succeeded to be running", "set name", setName)
	return nil
}

func (r *ZookeeperClusterReconciler) waitStsSuccess(ctx context.Context, log logr.Logger, cluster *qilitangtopv1.ZookeeperCluster, setName string) bool {
	newSet := &v1.StatefulSet{}
	...
	if err == nil {
		...
		if setStatus.IsReady() {
			...
			if ready {
				...
				_, index, _ := options.ParseClusterSetName(setName)
				if err = r.reConfigCluster(ctx, log, cluster, index); err != nil {
