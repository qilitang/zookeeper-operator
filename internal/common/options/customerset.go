...
func ParseClusterSetName(setName string) (clusterName string, index int, err error) {
	parts := strings.Split(setName, "-replica")
	if len(parts) != 2 {
		err = fmt.Errorf("invalid setName format")
		return
	}
	clusterName = parts[0]
	index, err = strconv.Atoi(parts[1])
	if err != nil {
		err = fmt.Errorf("invalid index: %v", err)
		return
	}
	return
}
