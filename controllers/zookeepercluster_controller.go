...
func (r *ZookeeperClusterReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	newCluster.Status.Replicas = 0
	newCluster.Status.ReadyReplicas = 0

	if err := r.syncReplicas(ctx, newCluster); err != nil {
		log.Info(fmt.Sprintf("cluster %s has be deleted, skip", cluster.Name))
		return ctrl.Result{}, err
	}

	// update cluster spec and status
	if err := r.UpdateCluster(ctx, newCluster); err != nil {
		return ctrl.Result{}, err
	}
	return ctrl.Result{}, nil
}
